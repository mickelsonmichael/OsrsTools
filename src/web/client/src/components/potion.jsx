...
export default class Potion extends React.Component {
  ...
  calculateSeedXp(seeds) {
    let xp =
      seeds *
      this.props.yield *
      (this.props.potion.cleanXP + this.props.potion.xp);
    ...
    if (this.props.showGrimy) {
      rows.push(
        <tr key="grimy">
          ...
          <td>
            <img
              src={potionRootUrl + this.props.potion.potionFilename}
              alt={this.props.potion.name}
              width="12"
              height="18"
              className="row_img"
            />
          ...
          <td>
            <img
              src={herbRootUrl + this.props.potion.cleanFilename}
              alt={"Grimy " + this.props.potion.herb}
              height="18"
              width="22"
              className="row_img"
            />
          ...
          <td>
            <input
              ...
              onChange={(e) => this.calculateCleanXp(Number(e.target.value))}
              min="0"
            />
          </td>
          <td>
            <NumberFormat
              value={this.state.herbsXp}
              displayType={"text"}
    ...
    if (this.props.showClean) {
      rows.push(
        <tr key="clean">
          ...
          <td>
            <img
              src={herbRootUrl + this.props.potion.grimyFilename}
              alt={this.props.potion.herb}
              height="18"
              width="22"
              className="row_img"
            />
          ...
          <td>
            <input
              type="number"
              data-type="dirty"
              value={this.state.grimy}
              onChange={(e) => this.calculateGrimyXp(Number(e.target.value))}
              min="0"
            />
          </td>
          <td>
            <NumberFormat
              value={this.state.grimyXp}
              displayType={"text"}
              thousandSeparator={true}
              decimalScale={1}
              allowNegative={false}
      ...
      rows.push(
        <tr key="seed">
          ...
          <td>
            <img
              src={herbSeedUrl}
              alt={this.props.potion.herb + " Seed"}
              height="18"
              width="22"
              className="row_img"
            />
          ...
          <td>
            <input
              ...
              onChange={(e) => this.calculateSeedXp(Number(e.target.value))}
              min="0"
            />
            (avg. yield:{" "}
            <NumberFormat
